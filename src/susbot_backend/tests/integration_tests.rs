...
use pocket_ic::common::rest::{CanisterHttpReply, CanisterHttpRequest, CanisterHttpResponse, MockCanisterHttpResponse, RawMessageId};
...
fn setup() -> (PocketIc, Principal) {
    std::env::set_var("POCKET_IC_BIN", POCKET_IC_BINARY_PATH); // Path of the pocket-ic binary
...
fn test_successful_scan() {
    ...
    let vec1 = serde_json::to_vec(&resp).unwrap();
    ...
    assert!(reply.is_ok());
    // let http_response: Result<HttpRequestResult, (RejectCode, String)> =
    let res =    decode_one::<ScanResult>(&reply.unwrap());
    assert!(res.is_ok());
    let finalres = res.unwrap();
    println!("{}", finalres.score)
}

fn start_processing(pic: &PocketIc, backend_canister: Principal, input_token: &str) -> (RawMessageId, CanisterHttpRequest) {
    let call_id = pic
        .submit_call(
            backend_canister,
            Principal::anonymous(),
